...
class ReconciliationEngine:
    ...
    def _match_processor_to_crm_row(self, proc_row, crm_row, proc_config, crm_idx):
        ...
        crm_email_raw = safe_lower_strip(crm_row.get('crm_email'))
        crm_emails = [safe_lower_strip(e) for e in crm_email_raw.split(',')]
        ...
        if proc_name == 'shift4':
            ...
            if not (email_prefix or first_prefix or last_prefix):
                return None
            email_sim = 1 if email_prefix or first_prefix or last_prefix else 0
            ...
            if not (email_prefix or first_match or last_match):
                return None
            email_sim = self.enhanced_email_similarity(crm_email_raw, proc_email)
            ...
            email_sim = self.enhanced_email_similarity(crm_email_raw, proc_email)
        if proc_config.require_email and email_sim < 0.2 and not last4_match:
            return None
        if email_sim < 0.3:
            return None
        if proc_config.require_last4 and not last4_match and email_sim < 0.5:
            return None
        status_abs_tol = 0.1
        ...
        match = {
            ...
            'comment': comment,
            'matched_proc_indices': [proc_row.name],
    ...
    def _cross_processor_last_chance(self, crm_df, processor_df, used_crm, used_proc, matches):
        ...
        for proc_idx, proc_row in unmatched_proc_rows.iterrows():
            ...
            for crm_idx in candidate_crm_indices:
                ...
                if (crm_pname_lower == 'safechargeuk' and proc_pname_lower == 'safecharge') or (
                        crm_pname_lower == 'safecharge' and proc_pname_lower == 'safechargeuk'):
                    continue

                if crm_pname_lower == proc_pname_lower:
                    continue
    ...
    def _cross_regulation_matching(self, crm_df, processor_df, used_crm, used_proc, matches):
        ...
        for crm_idx in unmatched_crm_indices:
            ...
            if crm_reg_lower == 'uk':
                target_proc = 'safecharge'
                match_func = self._match_standard_row
            else:
                target_proc = 'safechargeuk'
    ...
    def _flag_warning(self, matches, processor_df):
        ...
        for proc_i in unmatched_proc_shift4_indices:
            ...
            for crm_i in unmatched_crm_indices:
                ...
                crm_emails = [e.strip() for e in crm_email_raw.split(',')]
    ...
    def make_cancelled_rows(self, full_crm_df):
        ...
        for _, row in full_crm_df.loc[mask].iterrows():
            ...
            out['regulation'] = row.get('regulation', '')
    ...
    def _match_standard_row(self, crm_row, proc_dict, last4_map, used, proc_config, skip_proc_check=False,
        ...
        if self.config.get('force_skip_proc_check', False) or skip_proc_check:
            ...
            accepted_proc = ['safecharge', 'safechargeuk'] if crm_proc_name in ['safecharge', 'safechargeuk'] else [
                crm_proc_name]
            indices = [i for i in proc_dict if
...
def run_cross_processor_matching(date_str: str, exchange_rate_map: dict) -> None:
    ...
    print(
        f"[Cross-processor] Finished: {len(row_within_matches)} within ROW, {len(uk_within_matches)} within UK, {len(row_cross_matches)} cross to ROW cross file, {len(uk_cross_matches)} cross to UK cross file.")