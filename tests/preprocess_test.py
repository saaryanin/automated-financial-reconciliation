...
def combine_processed_files(
    ...
    def group_crm_withdrawals(df, exchange_rate_map):
        ...
        # Fill PSP for cancellations if empty, from Internal Comment
        df['PSP name'] = df['PSP name'].fillna('').astype(str).str.strip().str.lower()
        df['Internal Comment'] = df['Internal Comment'].fillna('').astype(str).str.strip().str.lower()
        mask = (df['PSP name'] == '') & (df['Name'].str.lower() == 'withdrawal cancelled')
        df.loc[mask, 'PSP name'] = df.loc[mask, 'Internal Comment'].replace(PSP_NAME_MAP)
