...
# --------------------------------------------------------------------------- #
def _write_cross_matches(matches: List[Dict], date_str: str, regulation: str) -> None:
    ...
    df_new = df_new.reindex(
        columns=[c for c in desired_order if c in df_new.columns or c == "crm_type"]
    )

    # Clean last-4 the same way the normal matcher does
    if "proc_last4" in df_new.columns:
        df_new["proc_last4"] = (
            df_new["proc_last4"].astype(str).str.replace(r"\.0$", "", regex=True)
        )
...
def _cross_match_one_way(
    ...
    if crm_reg == "row":
        ...
        proc_pool = proc_pool[
            ~proc_pool["proc_processor_name"].str.lower().isin(uk_only_processors)
        ]
    ...
    processor_df["proc_amount"] = pd.to_numeric(
        processor_df["proc_amount"], errors="coerce"
    )
    ...
    if proc_reg == "uk":
        processor_df_for_match["proc_processor_name"] = processor_df_for_match[
            "proc_processor_name"
        ].str.replace("safechargeuk", "safecharge", case=False)
    ...
    for m in raw_matches:
        if m["match_status"] == 1:
            if (
                proc_reg == "uk"
                and m.get("proc_processor_name", "").lower() == "safecharge"
            ):
            ...
            if "crm_row_index" in m:
                ...
                print(
                    f"Warning: Missing crm_row_index for match with crm_email {m['crm_email']}"
                )
            # Map matched_proc_indices (reset) to original_proc_index
            m["original_proc_indices"] = [
                processor_df.loc[p, "original_proc_index"]
                for p in m["matched_proc_indices"]
            ]
...
def run_cross_regulation_matching(date_str: str, exchange_rate_map: dict) -> None:
    ...
    # ------------------------------------------------------------------- #
    row_file = (
        TEMP_DIR / "ROW" / "data" / "lists" / date_str / "row_withdrawals_matching.xlsx"
    )
    uk_file = (
        TEMP_DIR / "UK" / "data" / "lists" / date_str / "uk_withdrawals_matching.xlsx"
    )
    ...
    if uk_to_row:
        ...
        matched_original_proc_idxs = [
            p for m in uk_to_row for p in m["original_proc_indices"]
        ]
    ...
    if row_to_uk:
        ...
        matched_original_proc_idxs = [
            p for m in row_to_uk for p in m["original_proc_indices"]
        ]
