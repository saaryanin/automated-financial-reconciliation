...
def handle_withdrawal_cancellations(df):
    if "Name" not in df.columns:
        return df
    # Normalize columns for grouping and matching
    key_cols = ['First Name (Account) (Account)', 'Last Name (Account) (Account)', 'Email (Account) (Account)', 'CC Last 4 Digits', 'TP Account']
    for col in key_cols:
        if col in df.columns:
            df[col] = df[col].fillna('').astype(str).str.strip().str.lower()  # Lower for case-insensitivity
    if 'Amount' in df.columns:
        df['Amount'] = pd.to_numeric(df['Amount'], errors='coerce').fillna(0).abs()  # Ensure positive
    ...
    for keys, group in grouped:
        if group.empty:
            continue
        withdrawals = group[group['Name'] == 'withdrawal']
        cancellations = group[group['Name'] == 'withdrawal cancelled']
        sum_withdrawals = withdrawals['Amount'].sum()
        sum_cancellations = cancellations['Amount'].sum()
        if abs(sum_withdrawals - sum_cancellations) < 1e-6:  # Net zero, remove entire group
            to_drop.update(group.index.tolist())
        else:
            # For unmatched cancellations, keep amount positive and set 'Name' to 'Withdrawal Cancelled'
            unmatched_cancels = cancellations[~cancellations.index.isin(withdrawals.index)]
            for idx in unmatched_cancels.index:
                df.at[idx, 'Name'] = 'Withdrawal Cancelled'

    # Drop the matched groups where net is zero
...
def combine_processed_files(
    ...
    def group_crm_withdrawals(df, exchange_rate_map):
        ...
        if last4_nonblank:
            group_cols = ['CC Last 4 Digits', 'Email (Account) (Account)']
        else:
            group_cols = ['First Name (Account) (Account)', 'Last Name (Account) (Account)']
